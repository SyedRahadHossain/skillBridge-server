...
const updateStatus = async (bookingId: number, userId: string, role: string, status: BookingStatus) => {
    ...
    if (role === "tutor") {
        ...
        if (status !== "completed") throw new Error("Tutors can only mark bookings as completed!");
    }
...
const getAllBookings = async (query: {
    ...
    const { page, limit, skip, sortBy, sortOrder } = require("../../../helpers/paginationSortingHelper").default(query);
